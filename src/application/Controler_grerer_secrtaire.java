...
public class Controler_grerer_secrtaire  implements Initializable {
	...
	@FXML
	private TableView<seretairemodel> table;
	@FXML
	private TableColumn<seretairemodel, Integer> id;
	@FXML
	private TableColumn<seretairemodel, String> nom;
	@FXML
	private TableColumn<seretairemodel, String> email;
	@FXML
	private TableColumn<seretairemodel, String> adresse;

	@FXML
	private TextField search;
	public ObservableList<seretairemodel> data = FXCollections.observableArrayList();
	public static Connection con = DAO.Connexion.getConnexion();
	public static PreparedStatement preparStat;

	FilteredList<seretairemodel> filter = new FilteredList<seretairemodel>(data, e -> true);
	...
	public void	Test() {
		if (table.getSelectionModel().getSelectedItem() != null) {
			seretairemodel seretairemodel = table.getSelectionModel().getSelectedItem();
			EMT.setText(seretairemodel.getEmail());
			NAT.setText(seretairemodel.getNom());
			LNT.setText(seretairemodel.getPrenom());
			ADT.setText(seretairemodel.getAdress());
			IDTT.setText(String.valueOf(seretairemodel.getId()));
			PAT.setText(seretairemodel.getPassword());
...
public void Save() throws SQLException {
	...
	test_nom.setText("name  not valid");	
	 ...
	 test_password.setText("you have to use 8 characters ");
		...
		seretairemodel.setId(IIId);
		seretairemodel.setPassword(MotPasse);
		
		
			int status = DAO_CREATION_DU_SECRETAIRE.Modifier(seretairemodel);
			if (status == 2) {
	...
	public void SupprimerButtonAction(ActionEvent e) throws IOException, SQLException {
		if (table.getSelectionModel().getSelectedItem() != null) {
			...
			int status = DAO_CREATION_DU_SECRETAIRE.Supprimer(seretairemodel);
			if (status == 2) {
	...
	private void search(KeyEvent event) {
		...
		search.textProperty().addListener((observableValue, oldValue, newValue) -> {
			filter.setPredicate((java.util.function.Predicate<? super seretairemodel>) (seretairemodel seretairemodel) -> {
				if (newValue == null || newValue.isEmpty()) {
					return true;
				}
				String lowerCaseFilter = newValue.toLowerCase();
				if (seretairemodel.getNom().toLowerCase().contains(lowerCaseFilter)) {
					return true;
				} else if (seretairemodel.getEmail().toLowerCase().contains(lowerCaseFilter)) {
					return true;
				}
		...
		SortedList<seretairemodel> sortedData = new SortedList<>(filter);
	...
	public void updateTable() {
		...
		try {
			String requete = "SELECT * FROM SECRETAIREE";
			preparStat = con.prepareStatement(requete);
			ResultSet rs = preparStat.executeQuery();
			while (rs.next()) {
				data.add(new seretairemodel(rs.getString("NOM"), rs.getString("PRENOM"), rs.getString("EMAIL"),
